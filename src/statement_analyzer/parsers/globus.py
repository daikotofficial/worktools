...
class ParsedRow:
    ...
    def is_header(self) -> bool:
        text = " ".join(part for part in (self.serial, self.post_date, self.value_date, self.description) if part).upper()
        return "S/N" in text and "POST DATE" in text and "DESCRIPTION" in text

    def is_description_only(self) -> bool:
        return bool(self.description) and not self.has_date and not self.has_amounts


class GlobusStatementParser(StatementParser):
    bank_name = "globus"

    def can_parse(self, pdf_path: Path) -> bool:
        profile = detect_layout(pdf_path)
        return profile is not None and profile.key == "globus_statement"
    ...
    def parse(self, pdf_path: Path) -> list[Transaction]:
        ...
        for index, row in enumerate(rows):
            ...
            description_parts = [extra.description for extra in extras if extra.top < row.top]
            description_parts.append(row.description)
            description_parts.extend(extra.description for extra in extras if extra.top >= row.top)
            description = clean_text(" ".join(part for part in description_parts if part))

            transactions.append(
                Transaction(
                    transaction_date=parse_date(row.post_date),
    ...
    def _extract_metadata(self, pdf_path: Path) -> StatementMetadata:
        ...
        period_match = re.search(
            r"Summary Statement for,\s*(\d{2}-\d{2}-\d{4})\s*to\s*(\d{2}-\d{2}-\d{4})",
            first_page_text,
            flags=re.IGNORECASE,
        )

        return StatementMetadata(
            account_name=extract_regex(first_page_text, r"Account Name\s+(.+?)\s+Opening Balance"),
            ...
            total_debit=parse_decimal(extract_regex(first_page_text, r"Total Withdrawals\s+(-?[0-9,]+(?:\.\d{1,2})?)")),
            total_credit=parse_decimal(extract_regex(first_page_text, r"Total Lodgement\s+(-?[0-9,]+(?:\.\d{1,2})?)")),
    ...
    def _extract_page_rows(self, page: pdfplumber.page.Page, page_number: int) -> list[ParsedRow]:
        ...
        for group in grouped:
            ...
            row.serial = " ".join(columns["serial"]).strip()
            row.post_date = " ".join(columns["post_date"]).strip()
...
def extract_reference(description: str) -> str | None:
    match = re.search(r"\b(?:REF[:/ ]?)?([0-9]{10,})\b", description, flags=re.IGNORECASE)
